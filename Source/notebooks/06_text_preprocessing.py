...
#
# Data nằm ở `Source/data/raw/text/data/*.parquet` (train/test split).
...
# Fallback: nếu không đúng cấu trúc mặc định, thử tìm 2 file parquet train/test
_train_pq = DATA_RAW / 'train-00000-of-00001.parquet'
_test_pq = DATA_RAW / 'test-00000-of-00001.parquet'
if not (_train_pq.exists() and _test_pq.exists()):
    _candidates = [
        _SOURCE_DIR / 'data' / 'raw' / 'text',              # có thể đặt parquet trực tiếp ở đây
        _SOURCE_DIR.parent / 'data' / 'raw' / 'text' / 'data',
        _SOURCE_DIR.parent / 'Source' / 'data' / 'raw' / 'text' / 'data',
    ]
    for _cand in _candidates:
        if (_cand / 'train-00000-of-00001.parquet').exists() and (_cand / 'test-00000-of-00001.parquet').exists():
            DATA_RAW = _cand
            _train_pq = DATA_RAW / 'train-00000-of-00001.parquet'
            _test_pq = DATA_RAW / 'test-00000-of-00001.parquet'
            print(f"[INFO] Dùng đường dẫn dự phòng cho text parquet: {DATA_RAW}")
            break
    else:
        raise FileNotFoundError(
            "Không tìm thấy 2 file parquet train/test cho RAGTruth.\n"
            f"Đường dẫn mặc định: {_SOURCE_DIR / 'data' / 'raw' / 'text' / 'data'}\n"
            f"Đã thử: {[str(c) for c in _candidates]}"
        )

df_train = pd.read_parquet(_train_pq)
df_test = pd.read_parquet(_test_pq)
