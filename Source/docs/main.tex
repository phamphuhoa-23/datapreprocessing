...
\textbf{Phần 1 -- Ảnh:}
Việc chọn NWPU-RESISC45 với 45 lớp cân bằng là lý tưởng cho ablation study vì loại bỏ được yếu tố nhiễu từ class imbalance. Kết quả IncrementalPCA (88{,}2\% tại $n=800$) cho thấy đặc trưng pixel viễn thám có chiều thực sự rất cao -- đây là lý do tại sao t-SNE (phi tuyến) phân tách lớp tốt hơn PCA 2D. Hạn chế chính: k-NN trên pixel features không phản ánh được khả năng học đặc trưng sâu của CNN -- các kết luận về augmentation (Wilcoxon $p > 0{,}05$ cho mọi kỹ thuật) và color space có thể khác biệt đáng kể khi dùng feature extractor CNN như ResNet.

Một phát hiện thú vị: 224$\times$224 cho k-NN accuracy bằng 128$\times$128 (0{,}260) -- cho thấy pixel-level k-NN không hưởng lợi từ độ phân giải cao hơn; lý do chọn 224$\times$224 là tính tương thích với ImageNet pretrained models (downstream fine-tuning).

\textbf{Phần 2 -- Dữ liệu bảng:}
Tập IEEE-CIS có tỉ lệ missing cực cao (nhóm Identity: 84,5\%) và mất cân bằng lớp nghiêm trọng (28:1) -- đây là hai thách thức lớn nhất. Việc chọn \textbf{Mean} thay vì MICE là sự đánh đổi thực tế giữa hiệu năng (MICE: RMSE=97{,}35) và khả năng tính toán (MICE OOM với $>400$ cột $\times$ 590K hàng). Với dữ liệu thực tế quy mô lớn, cân nhắc dùng MissForest hoặc deep learning imputation như GAIN là hướng cải tiến tiềm năng.

Nhận xét quan trọng về Levene test: tất cả 5 phương pháp chuẩn hóa đều bác bỏ $H_0$ ($p \approx 0$) -- đây là kết quả kỳ vọng với dữ liệu có 399/400 cột không chuẩn, không phải điểm yếu của phương pháp. Tiêu chí phân biệt là Levene $F$ thấp nhất (RobustScaler = 1.527,92) và tính bền vững với outlier sau IQR clipping.

Về feature selection: sự đồng thuận giữa RF và GB trong top features (TransactionAmt, card1, V-groups) xác nhận đây là các tín hiệu phân biệt thực sự (không phải overfitting). Intersection RF$\cap$GB chiếm $>$60\% của Union, cho thấy hai bộ đặc trưng có thể thay thế nhau trong hầu hết các trường hợp.

\textbf{Phần 3 -- Văn bản:}
RAGTruth là tập dữ liệu thú vị và hiện đại (hallucination detection cho LLM RAG systems). Kết quả cho thấy TF-IDF vẫn cạnh tranh mạnh với Sentence Transformer trong các task phân loại nhị phân có tín hiệu từ vựng rõ ràng: LR + TF-IDF bigram (F1=0{,}7331) $>$ SVM + TF-IDF (0{,}7223) $>$ SVM + Word2Vec (0{,}6965) $>$ SVM + Sentence Transformer (0{,}6948). Nguyên nhân: Sentence Transformer được huấn luyện cho task semantic similarity chung -- với task hallucination detection, các tín hiệu surface-form (citation patterns, hedging language) mà TF-IDF nắm bắt tốt hơn có thể mang nhiều thông tin hơn ngữ nghĩa sâu.

Hạn chế: Zipf $\alpha = 1{,}63$ cho thấy corpus khá chuyên biệt (responses của LLM) so với văn bản tự nhiên thông thường ($\alpha \approx 1{,}0$); việc loại bỏ stop words không cải thiện phân loại (Cohen's $d = 0{,}017$) -- đây là dấu hiệu cho thấy cấu trúc ngữ pháp (thường ở stop words) có thể mang thông tin phân biệt hallucination.
